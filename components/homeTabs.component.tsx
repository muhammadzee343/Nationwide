        ...
        className={`relative self-start lg:min-h-[380px] xl:min-h-[450px]  
              ${
                ...
                "Electrical Installation Condition Report (EICR)"
                  ? "lg:bg-hero"
                  : activeService.title === "Energy Performance Certificate"
                  ? "lg:bg-hero_EPC"
                  : activeService.title === "Gas Safety Certificate"
                  ? "lg:bg-hero_gas_safety"
                  : activeService.title ===
                    "Building Survey & HomeBuyer Reports"
                  ? "lg:bg-hero_building_survey"
                  : ""
        ...
        <ul className="marker:text-lime marker:text-[20px] list-disc text-white lg:text-black ml-12 lg:ml-5">
          {activeService.points.map((ele, index) => {
            return (
              <li
                key={index}
                className="text-sm lg:text-[13px] xl:text-[14px] mb-[12px] xl:mb-[16px]"
              >
                {ele}
              </li>
            );
