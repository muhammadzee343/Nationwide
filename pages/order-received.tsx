...
const OrderReceived: React.FunctionComponent = ({ orderInfo, services }) => {
  const router = useRouter();

  const products = orderInfo.products.map((ele) => {
    const service = services.services.find((ser) => {
      return ser.name === ele.name;
    });
    let attributes: any = "";
    if (service.service_attributes) {
      attributes = Object.entries(service.service_attributes).map(
        ([key, value]) => ({
          [key]: value,
        })
      );
  ...
  const priceData = [
    {
      title: "SUBTOTAL",
      value: `$${
        parseInt(orderInfo.total_amount) + parseInt(orderInfo.total_discount)
      }`,
    },
  ];
        ...
        <div className="flex flex-row mr-10 pt-5 pb-5 md:p-5">
          <div className="md:border-r border-gray-500 border-dotted md:pr-10">
            ...
            <p className="font-bold">
              {data.title !== "TOTAL:"
                ? data.value
                : "£" + parseFloat(data.value.replace("$", "")).toFixed(2)}
            </p>
          </div>
        </div>
      );
    ...
    <div className="w-full flex flex-col justify-center items-center">
      ...
      <div className="w-full flex flex-wrap justify-center items-start sm:gap-x-22 mt-14 mb-14">
        <div className="w-full lg:w-[90%] flex flex-col justify-center pt-[35px] pb-[10px] px-2">
          ...
          <div className="flex flex-row">{orderDetail}</div>
          <div>
              ...
              <div className="w-full py-3">
                {products.map((item, index) => {
                  const propertyType: any =
                    attributes.property_type.options.find(
                      (data) => data.value === item.property_type
                    );
                  return (
                    <div
                      className={`w-full flex flex-row py-4 ${
                        index != orderInfo.products.length - 1 &&
                        "border-b-4 border-dark-blue"
                      }`}
                    >
              ...
              {priceData.map((item, index) => {
                return (
                  <div className={`w-full flex flex-row text-lg font-semibold`}>
                    <div
                      className={`w-2/4 bg-dark-blue py-3 text-white ${
                        index === 0 && "border-t-2"
                    ...
                    >
                      <p className="text-center">
                        {item.title === "PAYMENT METHOD"
                          ? item.value
                          : "£" +
                            parseFloat(item.value.replace("$", "")).toFixed(2)}
                      </p>
                    </div>
                  </div>
                );
      ...
      {orderData[3].value === "Bank Transfer" && (
        <div className="w-full lg:w-[90%] flex flex-col justify-center pb-[25px] px-2">
          <p>
            Note: Your order is on hold, pending receipt of the payment as shown
            above. we accept payments online, over the phone and through bank
            Transfer.
          </p>
          <br />
          ...
          <br />
          <p className="text-xl md:text-2xl xl:text-3xl font-semibold">
            Nationwide Services-UK Limited:
          </p>
...
export const getServerSideProps = async (context) => {
  ...
  const res = await fetch(
    `${process.env.BASE_URL_DEV}/orders/${query.order_id}`
  );
